...
/*
 * Mainly copied from
 * https://github.com/bdhirsh/pytorch_open_registration_example/blob/master/cpp_extensions/open_registration_extension.cpp
...
// This basic implementation get num of devices of the custom backend
int get_device_count() {
  std::cout << "Custom get_device_count() called!" << std::endl;
  return /* device num = */ 1;
}

// Here, we're exposing a custom device object that corresponds to our custom
// backend. We do this using pybind: exposing an
// "extension_name.getDeviceCount()" function in python, that's implemented in
// C++. The implementation in this file maps directly to the `PrivateUse1`
// device type.
PYBIND11_MODULE(libvpex, m) {
  m.def("custom_device", &get_custom_device, "get custom device object");
  m.def("getDeviceCount", &get_device_count, "get custom device num");
}
